...
class DeliusAddressIdHandler(private val addressRepository: AddressRepository) {
  ...
  @Transactional
  fun patchAddress(event: ProbationOffenderAddressCreated) {
    val existingAddressEntity = addressRepository.findByUpdateId(UUID.fromString(event.additionalInformation.cprAddressId))!!
