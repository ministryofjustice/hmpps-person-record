...
class SysconNationalityController(
  ...
  fun PersonEntity.processUpsert(nationality: PrisonNationality) {
    val person = Person.from(this).apply {
      nationalityNotes = nationality.notes
      nationalities = listOf(NationalityCode.fromPrisonCode(nationality.nationalityCode)).mapNotNull { it }
    }
...
@Profile("!preprod & !prod")
@PreAuthorize("hasRole('${PERSON_RECORD_SYSCON_SYNC_WRITE}')")
class SysconNationalityControllerDev(
  private val personRepository: PersonRepository,
  private val personService: PersonService,
) {
  @Operation(description = "Upserts a nationality record on a matching prison number. Role required is **$PERSON_RECORD_SYSCON_SYNC_WRITE**.")
  ...
  fun PersonEntity.processUpsert(nationality: PrisonNationality) {
    ...
    personService.processPerson(person, setOf(PseudonymEntity::class)) { this }
  }
