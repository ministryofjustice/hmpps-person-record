...
class VettingPersonSearchControllerIntTest : E2ETestBase() {
...
//    }
    ...
    @Test
    fun `single cluster - adfasfasft`() {
      val prisonNumber1 = randomPrisonNumber()
      val crn1 = randomCrn()
      val cluster = createPersonKey()
        .addPerson(createPerson(createRandomPrisonPersonDetails(prisonNumber1)))
        .addPerson(createPerson(createRandomProbationPersonDetails(crn1)))
      val strongestMatchPersonEntity = cluster.personEntities.first { it.prisonNumber == prisonNumber1 }
      val strongestPersonPrimaryPseudonym = strongestMatchPersonEntity.pseudonyms.first { it.nameType == NameType.PRIMARY }

      val personSearchResponse = webTestClient.post()
        .uri("/person/vetting/search")
        .bodyValue(
          VettingPersonSearchRequest(
            firstName = strongestPersonPrimaryPseudonym.firstName!!,
            lastName = strongestPersonPrimaryPseudonym.lastName!!,
            dateOfBirth = strongestPersonPrimaryPseudonym.dateOfBirth!!,
          ),
        )
        .authorised(roles = listOf(API_VETTING_SEARCH_ONLY))
        .exchange()
        .expectBody<VettingPersonSearchResponse>()
        .returnResult()
        .responseBody!!
