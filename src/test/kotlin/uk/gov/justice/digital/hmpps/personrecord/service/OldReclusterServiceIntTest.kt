...
import uk.gov.justice.digital.hmpps.personrecord.service.message.OldReclusterService
...
class OldReclusterServiceIntTest : MessagingMultiNodeTestBase() {
  ...
  @Autowired
  private lateinit var oldReclusterService: OldReclusterService
  ...
  fun `should log event if cluster needs attention`() {
    ...
    oldReclusterService.recluster(personKeyEntity)
  ...
  fun `should not recluster when single record that does not match`() {
    ...
    oldReclusterService.recluster(person.personKey!!)
  ...
  fun `should recluster when single record matches to one other cluster`() {
    ...
    oldReclusterService.recluster(cluster1)
