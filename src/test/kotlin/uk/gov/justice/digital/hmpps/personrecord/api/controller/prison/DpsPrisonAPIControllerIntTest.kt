...
class DpsPrisonAPIControllerIntTest : WebTestBase() {
  ...
  inner class SuccessfulProcessing {
    ...
    fun `prison religion data exists - returns correct response for religion history`() {
      ...
      assertThat(actualResponseBody.religionHistory).usingRecursiveComparison().isEqualTo(listOf(actualPrisonReligionEntity).map { PrisonReligion.from(it) })
    }
    ...
    fun `should add list of additional identifiers to the prison record`() {
      ...
      val responseBody = webTestClient.get()
        .uri(prisonDpsApiUrl(personOne.prisonNumber))
        .authorised(listOf(API_READ_ONLY))
        .exchange()
        .expectStatus()
        .isOk
    ...
    fun `should return redirect when the requested prison record has been merged`() {
      ...
      webTestClient.get()
        .uri(prisonDpsApiUrl(sourcePrisonNumber))
        .authorised(listOf(API_READ_ONLY))
        .exchange()
        .expectStatus()
        .is3xxRedirection
