...
class JoinClustersOnUpdateE2ETest : MessagingTestBase() {
  ...
  @Test
  fun `should create 2 people who do not match followed by a third which matches both`() {
  ...
  @Test
  fun `should create 2 people who do not match followed by a third which matches both, picks one and then matches both on update (delete me later)`() {
    ...
    update C, matches A and B, merges B into A
     */
    // actual
    /*
    create A
    create B, no match
    create C, matches A and B, picks A
    update C, matches A and B, no change to B :-(
     */
  ...
  @Test
  fun `should create 2 people, one with cro and pnc, one with pnc, which match`() {
    val firstCrnWithPnc = randomCrn()
    val secondCrnWithCro = randomCrn()
