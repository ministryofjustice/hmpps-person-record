...
class ProbationMergeEventListenerE2ETest : E2ETestBase() {
  ...
  fun `should delete record when cluster is part of a review`() {
    ...
    // 4. delete record
    publishProbationPersonDeletedEvent(PROBATION_PERSON_DELETED, sourceCrn)
  ...
  fun `when a cluster has multiple reviews, all are deleted when the cluster is deleted`() {
    ...
    publishProbationPersonDeletedEvent(PROBATION_PERSON_DELETED, targetCrn)
    targetPerson.assertPersonDeleted()
    publishProbationPersonDeletedEvent(PROBATION_PERSON_DELETED, person3Crn)
  ...
  fun `processes offender merge event with records on same cluster`() {
    ...
    val sourcePersonDetails = createRandomProbationPersonDetails().copy(masterDefendantId = sourceMasterDefendantId)
    val targetPersonDetails = createRandomProbationPersonDetails().copy(masterDefendantId = targetMasterDefendantId)
