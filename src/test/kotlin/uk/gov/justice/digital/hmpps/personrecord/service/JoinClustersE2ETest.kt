...
class JoinClustersE2ETest : E2ETestBase() {
  ...
  fun `two records match below fracture threshold then the next matches both above join so a cluster of 3 formed`() {
    ...
    val libraPersonRecord = awaitNotNull { personRepository.findByCId(cId) }
    ...
    val commonPlatformPersonRecord = awaitNotNull { personRepository.findByDefendantId(defendantId) }
    awaitAssert {
